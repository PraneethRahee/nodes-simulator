...
const BaseNode = React.memo(({type,title,icon,children,inputs = [],outputs = [],className,isConnectable = true,id}) => {
  const colorClasses = {
  ...
  const inputHandles = createInputHandles(inputs, colorClasses.handle, isConnectable, type);
    ...
    <Card className={cn(
      ...
      <CardHeader className={cn("p-3 pb-2 rounded-t-lg", colorClasses.header)}>
        <div className="flex items-center justify-between">
          ...
          <NodeDeleteButton nodeId={id || 'unknown'} nodeTitle={title} onNodeDelete={(nodeId) => {
